...
<html lang="en" class="{{ auth()->user()->role_id != 1 ? 'app_theme_h' : '' }}">
<head>
    <meta charset="UTF-8">
...
<body class="disable_transitions sidebar_main_open sidebar_main_swipe">
                ...
                <div class="uk-navbar-flip">
                    <ul class="uk-navbar-nav user_actions">
                        ...
                        <li data-uk-dropdown="{mode:'click',pos:'bottom-right'}">
                            <a href="#" class="user_action_image"><img class="md-user-image" src="{{ auth()->user()->image ? asset(auth()->user()->image) : asset('backend/assets/img/default-avatar.png') }}" alt=""/></a>
    ...
    <aside id="sidebar_main">
        ...
        <div class="sidebar_main_header">
            <a href="{{ url('/') }}" target="_blank" class="sSidebar_hide sidebar_logo_large">
                <img class="logo_regular" src="{{ asset('backend/assets/img/rms.png') }}" alt="" height="15">
            </a>
            <a href="{{ url('/') }}" target="_blank" class="sSidebar_show sidebar_logo_small">
                <img class="logo_regular" src="{{ asset('backend/assets/img/rms.png') }}" alt="" height="32">
            </a>
        </div>
        
        <div class="menu_section">
            @if (auth()->user()->role_id == 1)
                @include('layouts/sidenav_admin')
            @elseif (auth()->user()->role_id == 3)
                @include('layouts/sidenav_teacher')
            @else
                @include('layouts/sidenav_student')
            @endif
        </div>
    </aside>
